...
export class NotFoundComponent implements OnChanges, OnInit, OnDestroy {
	...
	private _name: string = this._translateService.instant( 'CORE.LABEL.RESULTS' );
	...
	ngOnChanges( changes: SimpleChanges ) {
		if ( changes.name ) this.name ||= _.toLower( this._name );
	 ...
	 */
	private _init() {
		const name: string = ( this.name !== _.toUpper( this.name ) ? _.toLower( this.name ) : this.name )
			|| _.toLower( this._name );

		switch ( this.mode ) {
			case 'empty':
				this.image = this.emptyImage ??= 'assets/images/icons/empty.png';
				this.imageSize = [ this.emptyImageSize?.[ 0 ], this.emptyImageSize?.[ 1 ] || this.emptyImageSize?.[ 0 ] ];
				this.title = this.emptyTitle ??= this._translateService.instant( 'CORE.MESSAGE.NO_DATA', { name } );
				this.description = this.emptyDescription;
				break;
			case 'error':
				this.image = this.errorImage ??= 'assets/images/icons/error.png';
				this.imageSize = [ this.errorImageSize?.[ 0 ], this.errorImageSize?.[ 1 ] || this.errorImageSize?.[ 0 ] ];
				this.title = this.errorTitle ??= this._translateService.instant( 'CORE.MESSAGE.COULD_NOT_LOAD_RESOURCES', { name } );
				this.description = this.errorDescription ??= this._translateService.instant( 'CORE.MESSAGE.COULD_NOT_LOAD_RESOURCES_DESCRIPTION', { name } );
				break;
			case 'custom':
